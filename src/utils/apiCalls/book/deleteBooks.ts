...
const deleteBook = async ({ id, bookListSetter, alertSetter, sortingColumn,
  sortByOrderIsASC }: DeleteBookBtnProps) => {
    try {
    ...
    // AWAIT: ensures UI thread is not blocked when long-running API calls are processed -- once task is completed, below code is executed
      ...
      getBooks(bookListSetter, sortingColumn, sortByOrderIsASC);
